...
const SETOR_TIPO_MAP: Record<string, TipoSetor> = {
  ...
  societario: TipoSetor.SOCIETARIO,
};
...
export default async function TVPage({
  ...
  return (
    <div className="min-h-screen bg-ds-ink text-white flex flex-col">
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-white/10">
        ...
        <div className="text-right">
          ...
          <p className="text-ds-info text-xs mt-0.5">Atualiza a cada 30s</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-8 px-8 py-3 bg-white/5 border-b border-white/10">
        {[
          { label: "Não iniciados", value: chamados.filter((c) => c.status === "NAO_INICIADO").length, color: "text-blue-300" },
          ...
          { label: "Total Ativo", value: chamados.length, color: "text-ds-info" },
        ].map((stat) => (
        ...
        {chamados.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {chamados.map((c) => (
              <div
                key={c.id}
                className="bg-white/5 border border-ds-info/30 rounded-xl p-4 hover:border-ds-info/60 transition-colors"
              >
                ...
                <div className="space-y-1.5 text-xs text-white/60">
                  ...
                  <p>
                    ...
                    {c.responsavel?.nome ?? (
                      <span className="text-ds-info italic">Aguardando</span>
                    )}
                  </p>
                  {(c.empresa || c.projeto) && (
                    <p>
